const input = `threerznlrhtkjp23mtflmbrzq395three
9sevenvlttm
3twochzbv
...
const testInput = `2eight2kqmmbsbjvxtvjhponesixtwonesn` //21

const calibrationInput = `two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
...
//     // ...
// ]
const NUM_WORD_ARRAYS = Object.entries(NUM_WORDS);

function stringStartsWithNumberWord(str) {
	for (let [word, number] of NUM_WORD_ARRAYS) { //destructure the array[0] and array[1]
...
const numbersInLines = calibrationInput.split('\n').map((line) => {
	const matchingNumbers = line
		.split('')
		.map((char, i) => {
		if (/\d/.test(char)) {
			return char;
		...
		return stringStartsWithNumberWord(restOfLineFromChar)
		
		}).filter(chars => chars !== null);

	// console.log(matchingNumbers)
	const firstNumber = matchingNumbers[0];
	// console.log(firstNumber)
	const lastNumber = matchingNumbers[matchingNumbers.length - 1];
